...
5. **Fix**: Apply code fixes for all actionable items
6. **Respond**: Reply to each fixed thread explaining what was changed
7. **Resolve**: Resolve each addressed thread via GraphQL mutation:
   ```graphql
   mutation { resolveReviewThread(input: {threadId: "<THREAD_NODE_ID>"}) { thread { id isResolved } } }
   ```
   Use `gh api graphql -f query='<mutation>'` for each thread node ID

Include thread node IDs (e.g. `PRRT_kwDO...`) in the triage output so they can be passed to the resolve mutation afterward.